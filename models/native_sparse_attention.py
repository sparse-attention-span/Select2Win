...
class SparseAttention(Module):
    ...
    def forward(
        ...
        if has_selected_kv_for_fine_attn:
            ...
            print(f"Input is: {inp.shape}, topk: {selected_block_indices.shape}")
...
class SparseAttentionMinimal(Module):
    def __init__(
        self,
        dim,
        dim_head,
    ...
    def get_top_k_blocks(self, q, k):
        # on shapenet, with 4 qheads and 2 kvheads
        # q: [1, 4, 8192, 16]
        # k: [1, 2, 8192, 16]
        # input: [1, 8192, 64]
        # topk : [1, 2, 8192, topk]

        # aggregate over query heads to match kv heads
        queries = rearrange(q, "b (kv kvperhead) n E -> b kv kvperhead n E", kv=self.kv_heads)
        queries = reduce(queries, "b kv kvperhead n E -> b kv n E", "mean")

        # compute importance scores
        keys = k
        similarity = einsum(queries, keys, "b kv n E, b kv m E -> b kv n m")
        similarity = torch.softmax(similarity * self.scale, dim=-1)

        # get topk
        topk_values, topk_indices = torch.topk(similarity, self.num_selected_blocks, dim=-1)

        topk_values = topk_values

        print(k.shape, topk_indices.shape)

        return topk_values.contiguous(), topk_indices.contiguous()
    ...
    def forward(
        ...
        if has_selected_kv_for_fine_attn:
            ...
            selected_importance_values, selected_block_indices = importance_scores.topk(num_selected, dim = -1)
            # selected_importance_values, selected_block_indices = self.get_top_k_blocks(q, k)
            gates = straight_through(selected_importance_values, 1.) if self.use_diff_topk else None
...
class SparseAttentionMinimal_buggy(Module):
    def __init__(
        self,
        dim,
        dim_head,
        ...
        block_size,
        topk,
        local_attn: Module | None = None,
    ):
        super().__init__()

        print(f"kv{kv_heads}, q{q_heads}")

        kv_heads = default(kv_heads, q_heads)
        assert kv_heads <= q_heads and divisible_by(q_heads, kv_heads)

        self.dim_head = dim_head
        self.kv_heads = kv_heads
        self.num_grouped_queries = q_heads // kv_heads
        self.scale = dim_head ** -0.5

        # scale
        dim_inner = dim_head * q_heads
        dim_kv_inner = dim_head * kv_heads
        qkv_split = (dim_inner, dim_kv_inner, dim_kv_inner)
        self.to_qkv = nn.Linear(dim, sum(qkv_split), bias = False)
        self.qkv_split = qkv_split
        ...
        # local attention
        self.use_local_attn = local_attn is not None
        self.local_attn = local_attn

        print(f"Using local attn: {self.local_attn is not None}")

        # selection related
        self.selection_block_size = block_size
        self.topk = topk

        # they combine the three sparse branches through a learned combine with sigmoid activation
        if self.use_local_attn:
            strategy_combine_mlp = nn.Linear(dim, 2 * q_heads)

            # init to sliding windows first, as network tends to pick up on local patterns first before distant ones

            nn.init.zeros_(strategy_combine_mlp.weight)
            strategy_combine_mlp.bias.data.copy_(tensor([-2., -2.] * q_heads))

            self.to_strategy_combine = nn.Sequential(
                strategy_combine_mlp,
                nn.Sigmoid(),
                Rearrange('b n (h s) -> b h n s', h = q_heads)
            )
    ...
    def forward(
        ...
        if self.use_local_attn:
            ...
            # combine strategies
            strategy_weighted_combine = self.to_strategy_combine(inp)
            strategy_concat = [fine_attn_out, local_attn_out]
            out = einsum(strategy_weighted_combine, stack(strategy_concat), 'b h n s, s b h n d -> b h n d')
        else:
            out = fine_attn_out
