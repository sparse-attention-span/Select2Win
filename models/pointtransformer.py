...
from .pt_serialization import encode
    ...
    def forward(self, point):
        ...
        if not self.enable_flash:
            ...
            feat = F.scaled_dot_product_attention(
                q, k, v, 
                attn_mask=self.rpe(self.get_rel_pos(point, order)) if self.enable_rpe else None,
                dropout_p=self._attn_drop).transpose(1, 2).reshape(-1, C)
        else:
...
class PointTransformer(PointModule):
    def __init__(
        self,
        in_channels,
        grid_size,
        ...
        self.out_dim = enc_channels[-1] if cls_mode else dec_channels[-1]
        self.grid_size = grid_size
        self._dim = dec_channels[0]
