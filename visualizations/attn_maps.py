...
# show_all = True # overrides settings below
# target_idx = [0, 2, 3, 4, 9,5 , 6, 7, 8]
...
# MAX=max(attn_map_data["value"].max() for attn_map_data in attn_maps.values())
# MIN=min(attn_map_data["value"].min() for attn_map_data in attn_maps.values())
...
#     ax.scatter(group_samples[:, 0], group_samples[:, 1], group_samples[:, 2], color=cmap((i % 20 / 20) * single_color))
...
#             ax.scatter(group_samples[:, 0], group_samples[:, 1], group_samples[:, 2], alpha=group_head_attn_map, color=cmap((j % 20 / 20) * single_color))
